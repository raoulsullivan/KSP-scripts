...
set desiredHeading to heading(270,90).
set step to 1.
stage.
lock long to ship:geoposition:lng.

when altitude > 10000 then {set desiredHeading to heading(270,60).}.
when altitude > 30001 then {set targetspeed to 2400.toggle AG1.}.
when altitude > 70001 then { 
	set step to 2.
}.
...
until step <> 2 {
	...
	until step = 3 {wait 1.clearscreen.print long.}.
}.
