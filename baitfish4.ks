...
set tInc to 105. //inclination of final orbit
...
//step 2 - gravity turn
when altitude > tA1 then {set step to "Gravity turn". lock tH to heading(tInc,90-(sqrt(altitude/tA2)*90)).}.
...
when ship:apoapsis >= tA2 then {
	...
	when time >= burnstart then {
		...
		when time >= burnstop then {
			lock throttle to 0.
			toggle RCS.
			set step to "Science! Badlands next...".
			clearscreen.
			when long > 48 then {
				toggle AG1.
				set step to "Science! Grasslands next...".
				when long > 89 then {
					toggle AG1.
					set step to "Science! Shore next...".
					when long > 121.2 then {
						toggle AG1.
						set step to "Science! Water next...".
						when long > 129 then {
							toggle AG1.
							set step to "Science! Deserts next...".
							when long < -170 then {
								when long > -140 then {
									toggle AG1.
								}.
