...
set step to "Circularise orbit - raise periapse".

set bd to body("Kerbin").
set currentradius to ship:obt:apoapsis+bd:radius.
set desiredSMA to currentradius.
set currentvelocityatap to sqrt((constant():g*bd:mass)*((2/currentradius)-(1/ship:obt:semimajoraxis))).
set desiredvelocityatap to sqrt((constant():g*bd:mass)*((2/currentradius)-(1/desiredSMA))).
set desireddv to desiredvelocityatap -currentvelocityatap.

set desireddi to ship:obt:INCLINATION*-1.
set dersireddv to (2*ship:velocity:orbit)*sin(desireddi/2).

//set x to node (time:seconds+ETA:APOAPSIS,0,0,desireddv).
set timefromperiapse to (ship:obt:argumentofperiapsis/360)*ship:obt:period.
print ("AN is this many secs before PE: "+timefromperiapse).
if (ETA:PERIAPSIS > timefromperiapse) {set nodetime to eta:periapsis - timefromperiapse. set x to node (time:seconds+nodetime,0,-desireddv,0).}.
else {set nodetime to eta:periapsis + timefromperiapse. set x to node (time:seconds+nodetime,0,-desireddv,0).}.
