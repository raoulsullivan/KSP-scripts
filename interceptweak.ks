...
run pidsetup(0.2,.5,0.2,.5,0.2,.5,5,5,5).
delete PIDsetup.
...
lock vx5 to VECTORCROSSPRODUCT(relativevelocity,VECTORCROSSPRODUCT(relativevelocity,target:direction:VECTOR)).
...
when vang(ship:facing:vector,df:vector) < 3 then {
	...
	when vang(target:direction:VECTOR,relativevelocity) < 3 then {
		...
		set pidly to 1.
		when alignposition:mag < 100 then {
			print "BRAKES!".
			set rpower to 0.5.
			when abs(relativevelocity:z) < 1 then {
				print "Over to RCS".
...
lock totalerror to abs(pE) + abs(yE).
...
set ue to true.
until step = "Done" {
	set vd10 to vecdrawargs(v(0,0,0),vx5,white,"Vectorcrossproduct",10,true).
