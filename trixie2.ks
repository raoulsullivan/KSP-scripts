...
when altitude > tA1 then {
	set step to "Gravity turn".
	lock tH to heading(tInc,90-(sqrt(altitude/tA2)*90))+ R(0,0,180).
}.
...
when ship:apoapsis >= tA2 then {
	...
	when ship:altitude > 70500 then {
		...
		set kpp to 0.05.
		set kdp to 0.5.
		//we going north or south? assume south.
		//so point a bit north
		set incadjust to -10.
		//when we've adjusted to min possible inclination, straighten up.
		when round(abs(ship:geoposition:lat),2) = round(ship:obt:inclination,2) then {
		...
		add x.
		set tH to x:BURNVECTOR:DIRECTION+ R(0+incadjust,0,90).
		...
		when time >= burnstart then {
			...
			lock throttle to 1. 
			when ship:periapsis >= tA2*0.95 then {
				lock throttle to 0.
